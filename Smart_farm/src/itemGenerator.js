...
function generateLayout(products) {
    ...
    products.forEach(product => {
        ...
        productDiv.className =
            "flex flex-col justify-self-center h-[450px] w-[300px] bg-red-300 rounded-xl p-2";

        const imageDiv = document.createElement("div");
        imageDiv.className = "h-[55%] w-[100%] bg-slate-200 rounded-xl  mb-2";
        const image = document.createElement('img');
        image.setAttribute('src', `${product.image}`)
        image.className = 'h-[100%] w-[100%] bg-slate-200 rounded-xl object-cover mb-2'
        imageDiv.appendChild(image)
        productDiv.appendChild(imageDiv);

        const infoDiv = document.createElement("div");
        infoDiv.className = "h-[43%] w-[100%] bg-slate-600 bg-transparent rounded-xl flex flex-col p-1";
